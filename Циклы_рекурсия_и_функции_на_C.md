...
Операторы `/` и `%` запрещены. Умножение `*` — можно.

Идея:

- **частное** — сколько раз удалось вычесть делитель из делимого;
- **остаток** — то, что осталось после вычитаний, либо `a - divide(a, b) * b`.
...
3. Перебирать делитель `d = 2, 3, 4, …` (до `sqrt(n)` или до самого `n` — оба варианта ок).
4. Пока `n` делится на `d` — запомнить `d` и заменить `n` на частное.
5. В ответе остаётся наибольший простой множитель.
...
В решениях ниже — **два варианта** кода: полный и упрощённый. Сдавать можно любой.
...
Комментарии — для разбора; перед сдачей можно оставить или укоротить.  
Ниже **два варианта** — сдай любой один.

#### Вариант A — полный

`divide` + `modulo`, цикл до `sqrt(n)`, защита от `INT_MIN` и переполнения `d*d`.
...
**Как работает:** читаем целое → берём модуль → разбиваем на множители, деля вычитанием. Каждый раз, когда делитель подходит, запоминаем его; в конце остаток (если > 1) — тоже простой множитель, и он самый большой. Нет простого делителя или плохой ввод → `n/a`.

#### Вариант B — попроще

Только `divide`; остаток через `n - divide(n, d) * d`; цикл `d <= n` без `sqrt` и без `INT_MIN`.

```c
#include <stdio.h>

/* Деление только вычитанием: 17/5 → вычитаем 5 три раза → 3.
 * / и % нельзя; * можно. */
int divide(int a, int b) {
  int q = 0;
  while (a >= b) {
...
/* Наибольший простой множитель. Нет ответа → -1. */
int largest_prime_divisor(int n) {
  int d = 2;
  int ans = 1;
  ...
  /* Пока есть что раскладывать: d бежит от 2 до текущего n.
   * Делится? → запомнили d и сжали n. Нет? → следующий d.
   * Остаток без %: n - divide(n, d) * d == 0 */
  while (d <= n) {
    if (n - divide(n, d) * d == 0) {
      ans = d;
      n = divide(n, d);
...
**Как работает:** одна вспомогательная функция — `divide`. Число раскладываем с `d = 2`: если делится — пишем `d` в ответ и делим `n`, иначе увеличиваем `d`. Когда `n` стало `1`, в `ans` уже наибольший простой множитель. Плохой ввод или `0`/`±1` → `n/a`.
...
**Как работает:** рекурсия с двумя базами (`0` и `1`); каждое следующее число — сумма двух предыдущих вызовов. Ввод `n < 0` или не целое → `n/a`. Для `21` получается `10946`.
...
**Как работает:** сначала та же таблица, что в Q4. Потом для каждой функции: находим max `y` на 42 точках, рисуем сетку 42×21 — в клетке `*`, если значение после масштаба попало в эту строку. Три графика подряд, без хранения массива (точку пересчитываем на лету).
